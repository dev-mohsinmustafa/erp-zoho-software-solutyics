...
import CreateSaleForm from "@/components/dashboard/CreateSaleForm";
...
const NewSale = async ({ initialData = {}, isUpdate = false }) => {
  const TAG = "NewSales.js"
  ...
  // 2- 
  const [categories, units, brands, warehouses, suppliers, items, products] = await Promise.all([
    categoriesData, unitsData, brandsData, warehousesData, suppliersData, itemsData, productsData
  ])
  ...
  //  console.log(TAG, "Warehouses:", warehouses);
  //  console.log(TAG, "Suppliers:", suppliers);

  return (
    <div>

      {/* Header */}
      <FormHeader title={isUpdate ? "Update Sale" : "New Sale"} href={"/dashboard/inventory/sales"} />
